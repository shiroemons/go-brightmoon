...
func TestMarisaArchive_OpenNonExistent(t *testing.T) {
...
func TestMarisaArchive_OpenTooSmall(t *testing.T) {
	tmpDir := t.TempDir()
...
func TestMarisaArchive_OpenInvalidHeader(t *testing.T) {
	tmpDir := t.TempDir()
...
func TestMarisaArchive_EnumBeforeOpen(t *testing.T) {
	archive := NewMarisaArchive()
...
func TestMarisaArchive_GetEntryBeforeOpen(t *testing.T) {
	archive := NewMarisaArchive()
...
func TestMarisaEntry_Methods(t *testing.T) {
	entry := &MarisaEntry{
		Offset: 100,
		Size:   200,
		Name:   "test.txt",
	...
	// MarisaEntry はサイズフィールドが1つのみ（圧縮なし）
