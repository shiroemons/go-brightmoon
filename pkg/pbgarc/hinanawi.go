...
// HinanawiEntry はHinanawiアーカイブ内のエントリを表します (C++版に合わせる)
type HinanawiEntry struct {
	Offset uint32 // 4 bytes: データオフセット
	Size   uint32 // 4 bytes: データサイズ
	Name   string // 可変長: ファイル名
	parent *HinanawiArchive
}

// GetEntryName はエントリ名を取得します
func (e *HinanawiEntry) GetEntryName() string {
	return e.Name
}

// GetOriginalSize は元のサイズを取得します
func (e *HinanawiEntry) GetOriginalSize() uint32 {
	return e.Size
}

// GetCompressedSize は圧縮後のサイズを取得します (Hinanawi形式は非圧縮)
func (e *HinanawiEntry) GetCompressedSize() uint32 {
	return e.Size
}
...
// HinanawiArchive はHinanawiアーカイブを表します
type HinanawiArchive struct {
	file     *os.File
...
func NewHinanawiArchive() *HinanawiArchive {
	return &HinanawiArchive{
		entries:  make([]HinanawiEntry, 0),
		curIndex: -1,
	}
}
...
// deserializeList は復号されたリストデータを解析します (C++版 Marisa/Hinanawi と同等)
func (a *HinanawiArchive) deserializeList(listBuf []byte, listCount, listSize, fileSize uint32) (bool, error) {
...
func (a *HinanawiArchive) EnumFirst() bool {
	if len(a.entries) == 0 {
		return false
	}
	a.curIndex = 0
...
func (a *HinanawiArchive) EnumNext() bool {
...
func (a *HinanawiArchive) GetEntryName() string {
...
// GetOriginalSize は元のサイズを取得します
func (a *HinanawiArchive) GetOriginalSize() uint32 {
	if a.curIndex < 0 || a.curIndex >= len(a.entries) {
		return 0
	}
...
func (a *HinanawiArchive) GetCompressedSize() uint32 {
	if a.curIndex < 0 || a.curIndex >= len(a.entries) {
		return 0
	}
	return a.entries[a.curIndex].Size
...
func (a *HinanawiArchive) GetEntry() PBGArchiveEntry {
	if a.curIndex < 0 || a.curIndex >= len(a.entries) {
		return nil
	}
